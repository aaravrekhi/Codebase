 ...
 * PP_5
 *
 * Purpose:     Sort binary data received from stdin based on specified criteria and perform a port scan attack analysis if required. 
 *              Print the sorted data as ASCII text to stdout and the time required to sort to stderr.
 *
 * Assumptions: The input stream is error-free for simplicity.
                The binary data follows the structure defined in snort5.c.
                The qsort() and clock() functions are available.
 *
 * Command line arguments: Two modes:
                                1. Sorting mode: Specify the field in the structure for sorting and the type of sort (qsort or bubble sort).
                                2. Port scan attack analysis mode: Perform a port scan attack analysis.
 *   
 * Known bugs: ?
 *
        ...
        int (*fcomp)(const void *, const void *))
{
    ...
    if (seqnum_list == NULL) {
        fprintf(stderr, "lab5: Cannot malloc array of size %d\n", records);
        exit(1);
    }
    for(i = 0; i < records; i++) {
        if (i != records - 1)
        ...
        int (*fcomp)(const void *, const void *))
{
    ...
    for (i = 0; i < records; i++) {
        for (j = 0; j < records - 1; j++) {
            //1 means they need to be sorted
...
{ 
    ...
    if (argc != 2 && argc != 3) { 
        fprintf(stderr, "Usage: lab5 sort-field [sort-type]\n");
    ...
    if (sort_field < 1 || sort_field > 5) {
        fprintf(stderr, "lab5: Invalid sort field %d\n", sort_field);
        exit(1);
    }
    if (sort_field == 5 && argc != 2) {
        fprintf(stderr, "lab5: Invalid port scan attack analysis %d\n", sort_field);
        exit(1);
    }
    if (argc == 3) {
        sort_type = atoi(argv[2]);
        if (sort_type < 0 || sort_type > 2) {
            fprintf(stderr, "lab5: Invalid sort type %d\n", sort_type);
            exit(1);
        }
    }
    ...
    if (num_items != 1) {
        fprintf(stderr, "lab5: Error fread failed to read number records %d\n", num_items);
        exit(1);
    }
    if (num_records < 2) {
        fprintf(stderr, "lab5: Error invalid number records %d\n", num_records);
        exit(1);
    }
    printf("lab5: num records %d\n", num_records);
    ...
    if (num_items != num_records) {
        fprintf(stderr, "lab5: failed to read all records %d\n", num_items);
        exit(1);
    }
    ...
    if (sort_type == 1)   // qsort
    {
        ...
        fprintf(stderr, "lab5: qsort time=%g for %d records\n", 
    ...
    if (sort_type == 2)   // bubble sort
    {
        ...
        fprintf(stderr, "lab5: bubble sort time=%g for %d records\n", 
                1000*((double)(end-start))/CLOCKS_PER_SEC, num_records);
    ...
    if (sort_field == 5) {
        fprintf(stderr,"lab5: no attacks detected for %d records\n", num_records);
        // OR
        ...
        fprintf(stderr,"lab5: discovered %d attacks\n", num_attacks);
    }
